...
func (bc *BlockStore) RunDownLoadBTCMoveLog() {
	...
	realpage, e := bc.GetBTCMoveLogTotalPage()
	if e != nil {
		fmt.Println(e)
		return
	}
	...
	if reqpage == 0 {
		...
		// Read last page data
		list, e := bc.GetBTCMoveLogPageData(reqpage)
		if e != nil {
			fmt.Println(e)
			return
		}
	...
	for {
		...
		// preservation
		e1 := bc.SaveBTCMoveLogPageData(lastpage, stores.SatoshiGenesisPageParseForShow(lastpageData))
		if e1 != nil {
